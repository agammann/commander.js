...
-  already used by option '${matchingOption.flags}'`);
   ...
   * @param {object} [parseOptions] - optionally specify argument style and environment
   * @param {string} [parseOptions.from] - where the args are from: 'node', 'user', 'electron'
   * @param {object} [parseOptions.env] - environment for options configured with .env(), defaults to process.env
   * @return {Command} `this` command for chaining
   */
  ...
  parse(argv, parseOptions) {
    ...
    this._parseCommand([], userArgs, parseOptions?.env ?? process.env);
  ...
  async parseAsync(argv, parseOptions) {
    this._prepareForParse();
    const userArgs = this._prepareUserArgs(argv, parseOptions);
    await this._parseCommand([], userArgs, parseOptions?.env ?? process.env);
...
- either make a new Command for each call to parse, or stop storing options as properties`);
 ...
 - ${executableDirMessage}`;
  ...
  _dispatchSubcommand(commandName, operands, unknown, env) {
    ...
    promiseChain = this._chainOrCall(promiseChain, () => {
      if (subCommand._executableHandler) {
        ...
        return subCommand._parseCommand(operands, unknown, env);
      }
    });
    return promiseChain;
  }
  ...
  _dispatchHelpCommand(subcommandName, env) {
    if (!subcommandName) {
      this.help();
    }
  ...
  _parseCommand(operands, unknown, env) {
    const parsed = this.parseOptions(unknown);
    this._parseOptionsEnv(env); // after cli, so parseArg not called on both cli and env
    ...
    if (
      ...
      return this._dispatchHelpCommand(operands[1], env);
    }
    ...
    if (this.parent?.listenerCount(commandEvent)) {
      ...
      if (this._findCommand('*')) {
        ...
        return this._dispatchSubcommand('*', operands, unknown, env);
      }
